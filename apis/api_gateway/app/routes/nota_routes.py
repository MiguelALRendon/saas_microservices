from flask import Blueprint, request
from app.middleware.jwt_middleware import gateway_auth_required
from app.middleware.csrf_middleware import csrf_required
from app.utils.sanitizer import sanitize_dict, sanitize_list
from app.utils.proxy import proxy_to_content
from config import Config

nota_bp = Blueprint('nota', __name__, url_prefix=f"{Config.API_PREFIX}/nota")


@nota_bp.route('/', methods=['GET'])
@gateway_auth_required
def get_notas():
    # Supports query params: fk_obra, fk_arco, page, per_page
    return proxy_to_content('/nota/')


@nota_bp.route('/<string:oid>', methods=['GET'])
@gateway_auth_required
def get_nota(oid):
    return proxy_to_content(f'/nota/{oid}')


@nota_bp.route('/', methods=['POST'])
@gateway_auth_required
@csrf_required
def create_nota():
    data = request.get_json() or {}
    return proxy_to_content('/nota/', sanitize_dict(data))


@nota_bp.route('/many', methods=['POST'])
@gateway_auth_required
@csrf_required
def create_many_notas():
    data = request.get_json() or []
    return proxy_to_content('/nota/many', sanitize_list(data) if isinstance(data, list) else data)


@nota_bp.route('/<string:oid>', methods=['PUT'])
@gateway_auth_required
@csrf_required
def update_nota(oid):
    data = request.get_json() or {}
    return proxy_to_content(f'/nota/{oid}', sanitize_dict(data))


@nota_bp.route('/many', methods=['PUT'])
@gateway_auth_required
@csrf_required
def update_many_notas():
    data = request.get_json() or []
    return proxy_to_content('/nota/many', sanitize_list(data) if isinstance(data, list) else data)


@nota_bp.route('/<string:oid>', methods=['DELETE'])
@gateway_auth_required
@csrf_required
def delete_nota(oid):
    return proxy_to_content(f'/nota/{oid}')
